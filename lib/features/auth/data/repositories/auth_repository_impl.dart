...
class AuthRepositoryImpl implements AuthRepository {
  ...
  Future<Either<Failure, void>> requestSignupOtp({
    ...
    try {
      ...
      final signupOtpRequest = OtpRequestModel(mobileNumber: mobileNumber, name: name);
  ...
  @override
  Future<Either<Failure, AuthSuccessEntity>> verifyOtp({ // AuthSuccessEntity import is in auth_repository.dart which this class implements

    required String mobileNumber,
    required String otp,
  }) async {
    try {
      final otpVerification = OtpVerificationModel(mobileNumber: mobileNumber, otp: otp);
      // TODO: Update remoteDataSource.verifyOtp to return AuthSuccessModel.
      final AuthSuccessModel authSuccessModel = await remoteDataSource.verifyOtp(otpVerification);
  ...
  Future<Either<Failure, TokensEntity>> refreshToken(String refreshToken) async {
    try {
      final refreshTokenRequest = RefreshTokenRequestModel(refreshToken: refreshToken);
      final tokensModel = await remoteDataSource.refreshToken(refreshTokenRequest);
